...
public interface PositionRepository extends JpaRepository<Position, Integer> {
	...
	@Query("select distinct p from Company c join c.positions p join p.skills s join p.technologies t where (p.title like %?1% or p.description like %?1% or p.profile like %?1% or s like %?1% or t like %?1% or c.commercialName like %?1%) and p.isFinalMode = 1 and p.isCancelled = 0 and p.deadline > CURRENT_DATE")
	...
	@Query("select distinct p from Hacker h join h.applications a join a.position p where a.status != 'REJECTED' and p.isFinalMode = 1 and p.isCancelled = 0 and p.deadline > CURRENT_DATE and h.id = ?1")
	Collection<Position> findPositionsApplicationsNotRejectedByHackerId(int hackerId);
